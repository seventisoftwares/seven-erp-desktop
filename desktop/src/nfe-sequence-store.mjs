...
export function createNfeSequenceStore({ dataDir }) {
  ...
  async function readState() {
    try {
      ...
      return parsed && typeof parsed === "object" ? { sequences: {}, reservations: {}, ...parsed } : { sequences: {}, reservations: {} };
    ...
    return serialized(async () => {
      ...
      if (existing) {
        if (!existing.issuedAt) {
          existing.issuedAt = existing.reservedAt || new Date().toISOString();
      ...
      state.sequences[sequenceKey] = { nextNumber: next + 1, updatedAt: reservedAt };
      await writeState(state);
      return { ...reservation };
    });
  }
  ...
  async function mark({ draftId, environment, series, status, accessKey = null, protocol = null }) {
    return serialized(async () => {
      ...
      const next = {
        ...
        protocol: protocol || current.protocol || null,
