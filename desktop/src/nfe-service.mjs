...
import { buildNfeXml, normalizeCnpj, validateCnpj, validateNfeDraft } from "./nfe-xml.mjs";
...
export function createNfeService({ dataDir, core, certificateVault, secretVault, fiscalDocumentStore, sequenceStore, appVersion = "1.0.0" }) {
  ...
  async function saveDraft(payload) {
    ...
    const draft = mergeDraft(data.draft, state.drafts[draftId], state.transmissions[draftId]);
  ...
  async function fiscalContext(draft) {
    ...
    if (!text(configuration.authorizationServiceUrl)) endpointBlockers.push("Informe a URL NFeAutorizacao4.");
    if (!text(configuration.returnAuthorizationServiceUrl)) endpointBlockers.push("Informe a URL NFeRetAutorizacao4.");
    if (!text(configuration.consultationServiceUrl)) endpointBlockers.push("Informe a URL NFeConsulta4.");
  ...
  async function transmit(draftId) {
    ...
    const reservation = await sequenceStore.reserve({
      draftId: draft.id,
      environment: context.environment,
      series: context.series,
      startingNumber: context.startingNumber,
    });
    ...
    try {
      const unsigned = buildNfeXml({
        ...
        issuedAt: reservation.issuedAt ? new Date(reservation.issuedAt) : new Date(),
        appVersion,
      });
  ...
  async function consultReceipt(draftId) {
    ...
    const context = await fiscalContext(draft);
    ...
    return jsonResponse(parsed.authorized ? 200 : 202, { ok: parsed.authorized, transmission: next, rawResponseStored: false });
  }
