...
import NfeSefazMirror from "./nfe-sefaz-mirror";
...
const saveSnapshot = (draftId: string, payload: AnyRow) => {
  ...
  localStorage.setItem(STORAGE_KEY, JSON.stringify(Object.fromEntries(Object.entries(current).sort((a,b) => String(b[1].savedAt).localeCompare(String(a[1].savedAt))).slice(0,200))));
};
...
export default function NfeMirrorCenterV2() {
  ...
  useEffect(() => {
    ...
    const wrappedFetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (rawUrl.startsWith("/api/nfe-drafts") && String(init?.method || "GET").toUpperCase() === "POST" && response.ok && typeof init?.body === "string") {
        ...
        } catch { /* nunca bloquear a operação fiscal por causa do espelho */ }
      }
      return response;
    };
  ...
  const load = async (preferredId = "") => {
    ...
    try {
      ...
      setDrafts(rows);
      setCompany(companyResponse.ok ? companyData.company || {} : {});
      setSnapshots(readSnapshots());
      setSelectedId((current) => {
        if (preferredId && rows.some((row: AnyRow) => row.id === preferredId)) return preferredId;
        if (current && rows.some((row: AnyRow) => row.id === current)) return current;
        return rows[0]?.id || "";
      });
  ...
  return <>{launcher}{open && <div className="nfe-mirror-overlay sefaz-mirror-mode" role="dialog" aria-modal="true" aria-label="Espelho da NF-e em padrão convencional">
    <div className="nfe-mirror-topbar no-print">
      <div><span>FISCAL · CONFERÊNCIA</span><h2>Espelho NF-e · padrão convencional</h2><p>Layout A4 no padrão visual do DANFE modelo 55. Antes da autorização permanece identificado como SEM VALOR FISCAL.</p></div>
    ...
    <div className="nfe-mirror-stage">
      {error ? <div className="mirror-empty-state"><b>Não foi possível abrir o espelho</b><span>{error}</span></div>
        : loading ? <div className="mirror-empty-state"><b>Carregando...</b><span>Montando a folha fiscal A4.</span></div>
        : !selected ? <div className="mirror-empty-state"><b>Nenhuma NF-e disponível</b><span>Salve uma NF-e para visualizar o espelho convencional.</span></div>
        : <><div className="mirror-snapshot-note no-print"><b>Layout convencional DANFE</b><span>{snapshot ? "Dados detalhados recuperados do snapshot desta NF-e." : "Alguns campos antigos podem ficar em branco até a nota ser salva novamente."}</span></div><NfeSefazMirror draft={selected} company={company} snapshot={snapshot} /></>}
    </div>
  </div>}</>;
}
