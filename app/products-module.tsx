...
const emptyItem = (): CatalogItem => ({
  ...
  costCents: 0, priceCents: 0, stockQuantityMilli: 0, minimumStockMilli: 0, status: "active", createdAt: "", updatedAt: "",
});
...
export default function ProductsModule({ onClose }: { onClose: () => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => (typeFilter === "all" || item.type === typeFilter) && (!needle || [item.sku, item.name, item.description, item.ncm, item.cest, item.serviceCode].join(" ").toLowerCase().includes(needle)));
  ...
  const openEdit = (item: CatalogItem) => { setEditing(item); setEditorOpen(true); setError(""); setNotice(""); };
  ...
  return <div className="catalog-workspace">
    ...
    {editorOpen && <div className="catalog-modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setEditorOpen(false)}><form className="catalog-editor" onSubmit={save}><header><div><span>{editing.id ? "EDITAR ITEM" : "NOVO ITEM"}</span><h2>{editing.id ? editing.name : "Cadastrar produto ou serviço"}</h2></div><button type="button" onClick={() => setEditorOpen(false)}>×</button></header>{error && <div className="catalog-message error">{error}</div>}<div className="catalog-form-scroll">
      ...
      <section><h3>Fiscal</h3><div className="catalog-form-grid"><label><span>NCM</span><input name="ncm" inputMode="numeric" maxLength={8} defaultValue={editing.ncm} placeholder="8 dígitos" /></label><label><span>CEST</span><input name="cest" inputMode="numeric" maxLength={7} defaultValue={editing.cest} /></label><label><span>Código do serviço</span><input name="serviceCode" defaultValue={editing.serviceCode} /></label></div></section>
